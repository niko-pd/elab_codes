...
gfortran -Wall -Wextra -Wconversion -pedantic -O -fcheck=all -g -fbacktrace -ffpe-trap=zero,overflow,underflow,invalid -ffree-line-length-none 3-all.f90 -o 3-all.x
...
thick_edge=1.5 # Ang, thickness of the "edge" region
...
do
	...
	echo "$vel $dt $min $displacement $ly $ny $exclude_period $thick_edge" | ../../3-all.x #&
	#sleep 20
